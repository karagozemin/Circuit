...
- `placeOrderFor` selector: `0x80054449`
...
This reads the pool-linked registry and the resolved authorization for the current pool at chain head. It checks only `placeOrderFor`; Circuit does not request cancel or reduce capabilities for IOC.
...
- Deploy the Circuit engine and verify its per-pool `placeOrderFor` authorization.

These steps require a funded testnet signer and deployed engine/handler addresses. No private key may be exposed through a `VITE_*` variable.
