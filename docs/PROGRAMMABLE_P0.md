...
**The new ladder code is locally tested, not yet deployed or live-proven.** The existing combined-setup deployment supports the earlier programs but cannot activate Ladder. Deploy the updated Engine + handler + linked SmartAccount and update the public frontend addresses to release Ladder. Do not label the existing historical live recording as a Ladder run or as proof that all three programs executed live.
