...
For the PRD-aligned autonomous path, deploy `CircuitSmartAccount(owner, engine)`, fund it with testnet collateral, and approve the current BinaryPool. The owner links it with `setExecutionAccount(strategyId, account)`. The Engine's policy checks remain in force, while the pool sees the smart account as `msg.sender` and therefore does not apply the foreign-contract allowlist to `placeBinaryOrder`.
