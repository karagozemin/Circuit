...
for(const signal of ['SIGINT','SIGTERM'] as const)process.on(signal,()=>{void(async()=>{await stopKeeper();await pause();proof.stage='interrupted and paused';await save();process.exit(1)})()})
try {
 ...
 while(Number((await client.getBlock()).timestamp)<Number(prep.market.expiry)+600) {
  ...
  if(runtime.round>=2 && runtime.status===2){
   ...
   const seedPosition=await client.readContract({address:prep.market.outcomeToken,abi:outcomeAbi,functionName:'balanceOf',args:[account.address,BigInt(prep.market.noId)]})
   if(seedPosition>0n){await send('seedRedeemApproval',prep.market.outcomeToken,outcomeAbi,'approve',[module,BigInt(prep.market.noId),seedPosition]);await send('seedRedeem',module,moduleAbi,'redeem',[0,zeroHash,prep.market.marketId,1,seedPosition])}
