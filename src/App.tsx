...
function App() {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        if (cursor !== undefined && cursor <= blockNumber) {
          ...
          if (!disposed) setActivity(previous => [...events.map(event => ({time:now(),title:event.eventName,detail:describeStrategyEvent(event.eventName,event.args),kind:'system' as const,hash:event.transactionHash})),...previous])
        }
  ...
  const sync = async () => {
    if(!window.ethereum || !walletState.snapshot || !strategyId || !walletConnected)return
    setControlBusy(true)
    try {
      const {configuredDeployment}=await import('./lib/contracts/activation')
      const deployment=configuredDeployment()
      if(!deployment)throw new Error('Circuit deployment is not configured.')
      const {syncStrategyTransaction}=await import('./lib/contracts/monitor')
      const result=await syncStrategyTransaction(window.ethereum,walletState.snapshot.address,deployment.engine,strategyId)
      setNotice(result?'Settlement or expiry synchronized from chain state.':'No sync is needed yet; the bound market has no new settlement or expiry transition.')
      if(result)appendActivity({time:now(),title:'Chain state synchronized',detail:'Permissionless backstop recovered the verified settlement or expiry transition.',kind:'success',hash:result.hash})
    }catch(error){setNotice(walletErrorMessage(error))}finally{setControlBusy(false)}
  }
