...
function App() {
  ...
  const activate = () => {
    ...
    setNotice('Wallet and market are ready, but no activation transaction was sent: CircuitEngine and its Reactivity subscription are not deployed yet.')
  }
  ...
  const pause = () => { setStatus('PAUSED'); setNotice('Strategy paused. No new orders can be submitted.'); appendActivity({ time: now(), title: 'Strategy paused', detail: 'User escape hatch', kind: 'system' }) }
  const resume = () => { setStatus('ARMED'); setNotice('Strategy resumed.'); appendActivity({ time: now(), title: 'Strategy resumed', detail: 'Listening for next valid trigger', kind: 'success' }) }
