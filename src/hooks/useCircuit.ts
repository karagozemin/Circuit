...
export function useCircuit(){
 ...
 const control=async(action:'pauseStrategy'|'resumeStrategy'|'cancelStrategy'|'sync')=>{if(controlLock.current)return;if(!window.ethereum||!walletConnected||!session){toast('Connect a wallet on Shannon to continue.','error');return}if(action!=='sync'&&!ownerConnected){toast('Connect the strategy owner wallet for this action.','error');return}
  controlLock.current=true;setOperation(action==='sync'?'Checking settlement':action==='pauseStrategy'?'Pausing strategy':action==='cancelStrategy'?'Ending circuit':'Resuming strategy');setTx(undefined)
